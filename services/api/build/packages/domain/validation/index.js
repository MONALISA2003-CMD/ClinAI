...
const EMAIL = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
export function isUuid(value) { return typeof value === 'string' && UUID.test(value.trim()); }
...
function label(field) { return field.label || field.key; }
export function validateRecord(contract, body, options = {}) {
    const issues = [];
    ...
    for (const field of fields) {
        ...
        if (['number', 'integer'].includes(type)) {
            ...
            if (!Number.isFinite(n))
                issues.push({ field: field.key, message: `${label(field)} must be a valid number.` });
            else if (type === 'integer' && !Number.isInteger(n))
                issues.push({ field: field.key, message: `${label(field)} must be a whole number.` });
        }
...
export function firstValidationMessage(issues) {
    return issues.length ? issues.map(x => x.message).join(' ') : '';
}
export const WORKFLOW_VALIDATION_CONTRACTS = {
