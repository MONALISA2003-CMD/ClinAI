...
let freeTierLock = Promise.resolve();
...
async function runGeminiAgent(deps, req, input, options, prompt, context, evidence, cacheKey, selectedModel) {
    ...
    let interaction = await geminiRequest({ model, input: history, system_instruction: baseSystem, tools, store: false, response_format: { type: 'text', mime_type: 'application/json', schema: responseSchema } });
    const toolsUsed = [];
    const calculations = [];
    let loop = 0;
