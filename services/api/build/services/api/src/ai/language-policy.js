...
export const SUPPORTED_CLINAI_LANGUAGES = Object.keys(POLICIES);
export function getLanguagePolicy(value) {
    const normalized = String(value || 'English').trim().toLowerCase();
    const aliases = { en: 'English', english: 'English', sw: 'Kiswahili', kiswahili: 'Kiswahili', swahili: 'Kiswahili', rw: 'Kinyarwanda', kinyarwanda: 'Kinyarwanda', lg: 'Luganda', luganda: 'Luganda', nyn: 'Runyankore', runyankore: 'Runyankore', 'runyankore-rukiga': 'Runyankore', runyankole: 'Runyankore', alz: 'Alur', alur: 'Alur' };
    return POLICIES[aliases[normalized] || 'English'];
}
export function languageInstruction(policy, detected) {
