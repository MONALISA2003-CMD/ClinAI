...
export function modelSupportsCapability(model, capability) {
    ...
    if (capability === 'image' || capability === 'audio' || capability === 'video')
        return model.modalities.includes(capability);
    if (capability === 'agentic')
        return model.roles.includes('agentic') && model.toolCalling;
    if (capability === 'medical')
        return model.roles.includes('medical-reasoning') || model.roles.includes('medical') || model.roles.includes('clinical-safety');
    if (capability === 'coding')
        return model.roles.includes('coding');
    if (capability === 'research')
        return model.roles.includes('research') || model.roles.includes('reasoning');
    if (capability === 'fast')
        return model.roles.includes('fast');
    return false;
}
function patientDataAllowed(model, allowPublic) {
    if (!model.patientDataEligible || model.noPersonalData)
        return false;
    if (model.publicEndpoint)
        return allowPublic;
    return true;
}
export function selectModel(opts) {
    ...
    if (capability && !compatible.length)
        return null;
    const pool = compatible;
    if (preferred && preferred.tier === 'free' && pool.some(m => m.id === preferred.id))
        return preferred;
...
function openAIHeaders(apiKey, provider) {
    return {
...
export async function callOpenAICompatible(model, prompt, system, schema, opts = {}) {
    const configured = configuredProviders();
    if (!FREE_ONLY_RUNTIME || model.tier !== 'free')
        throw Object.assign(new Error('Paid or non-free AI runtime models are disabled in ClinAI.'), { code: 'AI_NON_FREE_BLOCKED', statusCode: 503, provider: model.provider, model: model.id });
    let url = '';
    let key = '';
    ...
    if (!key || !configured[model.provider])
        throw Object.assign(new Error(`${model.label} is not configured.`), { code: 'PROVIDER_NOT_CONFIGURED', statusCode: 503 });
    await reserveFreeAIRequest(opts.pool || null, model.provider);
    const attachmentBlocks = (opts.attachments || []).map((a) => a.kind === 'image' ? ({ type: 'image_url', image_url: { url: a.dataUrl } }) : a.kind === 'audio' ? ({ type: 'input_audio', input_audio: { data: a.base64, format: a.format || 'wav' } }) : null).filter(Boolean);
    const userContent = attachmentBlocks.length ? [{ type: 'text', text: prompt }, ...attachmentBlocks] : prompt;
    const messages = opts.messages || [{ role: 'system', content: system }, { role: 'user', content: userContent }];
    const body = { model: model.id, messages, temperature: 0.2, max_tokens: opts.maxTokens || 4096 };
    ...
    if (!response.ok)
        throw Object.assign(new Error(data?.error?.message || `${model.label} request failed`), { statusCode: response.status, providerMessage: data?.error?.message || text, provider: model.provider });
    return { text: extractChatText(data), message: data?.choices?.[0]?.message || {}, usage: data?.usage || {}, provider: model.provider, model: model.id };
}
export function stableRequestKey(input) {
