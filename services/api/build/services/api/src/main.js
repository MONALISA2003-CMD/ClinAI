...
function canWrite(req) { return WRITE_ROLES.has(String(req.user?.role || '').toLowerCase()); }
...
function requireAuthorizedWrite(req) {
    if (PUBLIC_PREVIEW)
        throw Object.assign(new Error('Public preview is read-only. Sign in to the authorized ClinAI workspace to make changes.'), { statusCode: 403, code: 'PUBLIC_PREVIEW_READ_ONLY' });
    if (!canWrite(req))
...
app.get('/api/governance/:kind', async (req, reply) => {
    ...
    const r = await pool.query(`SELECT id,status,payload,created_at AS "createdAt",updated_at AS "updatedAt" FROM module_records WHERE organization_id=$1 AND module=$2 ORDER BY created_at DESC LIMIT 500`, [dbOrganizationId(req), module]);
...
app.get('/api/messages', async (req) => { if (!pool)
    return { data: store.messages || [], count: (store.messages || []).length }; const r = await pool.query(`SELECT id,status,payload,created_at AS "createdAt" FROM module_records WHERE organization_id=$1 AND module='messages' ORDER BY created_at DESC LIMIT 500`, [dbOrganizationId(req)]); return { data: r.rows.map((x) => ({ id: x.id, ...x.payload, status: x.status, createdAt: x.createdAt })), count: r.rowCount }; });
