...
export async function buildPatientIntelligenceLayer(query, organizationId, patientId) {
    const [criticalLabs, urgentTasks, overdueFollowups, delayedReferrals, immunizationsDue, activeRisks, recentResults, admissions, carePlans, reconciliation, finance, supply, publicHealth, longitudinalEvents, procedures, referrals] = await Promise.all([
    ...
    const gaps = [
        ...criticalLabs.map(x => ({ id: `lab:${x.id}`, patientId, gapType: 'critical-result-review', severity: 'critical', sourceModule: 'laboratory', title: `Critical laboratory result requires review`, evidence: x })),
        ...urgentTasks.map(x => ({ id: `task:${x.id}`, patientId, gapType: 'priority-task', severity: x.priority === 'critical' ? 'critical' : 'urgent', sourceModule: 'care-tasks', title: x.title || 'Priority care task remains open', dueAt: x.dueAt, evidence: x })),
        ...overdueFollowups.map(x => ({ id: `followup:${x.id}`, patientId, gapType: 'overdue-follow-up', severity: 'high', sourceModule: 'follow-up', title: x.reason || 'Follow-up is overdue', dueAt: x.dueAt, evidence: x })),
        ...delayedReferrals.map(x => ({ id: `referral:${x.id}`, patientId, gapType: 'delayed-referral', severity: 'high', sourceModule: 'referrals', title: x.reason || `Referral to ${x.destination || 'destination'} remains open`, evidence: x })),
        ...immunizationsDue.map(x => ({ id: `immunization:${x.id}`, patientId, gapType: 'immunization-due', severity: 'routine', sourceModule: 'immunization', title: `${x.vaccineName || 'Immunization'} is overdue`, dueAt: x.nextDueAt, evidence: x })),
        ...supply.filter(x => ['active', 'pending', 'ordered'].includes(String(x.status).toLowerCase()) && Number(x.availableQuantity || 0) < Number(x.quantity || 0)).map(x => ({ id: `supply:${x.id}`, patientId, gapType: 'medication-supply-gap', severity: 'high', sourceModule: 'inventory', title: `Recorded medication order has insufficient currently recorded stock`, evidence: x })),
        ...finance.filter(x => Number(x.balance || 0) > 0 && !['paid', 'cancelled', 'void'].includes(String(x.status).toLowerCase())).map(x => ({ id: `invoice:${x.id}`, patientId, gapType: 'unpaid-encounter-balance', severity: 'routine', sourceModule: 'finance', title: 'An invoice has a recorded outstanding balance', evidence: x })),
    ];
    const riskSignals = [
...
export async function buildClinicalVelocity(query, organizationId, days = 30) {
...
export async function buildValueBasedCare(query, organizationId, days = 30) {
...
export async function buildGovernanceSummary(query, organizationId) {
...
export async function recordSecurityEvent(pool, input) {
    if (!pool || !input.organizationId)
        return null;
...
export async function calculateClinicalMeasures(query, organizationId, start, end) {
    const rows = await safe(() => query(`
    SELECT
  ...
  `, [organizationId, start, end]), [{}]);
    const r = rows[0] || {};
    const ratio = (n, d) => Number(d || 0) ? Math.round((Number(n || 0) / Number(d)) * 10000) / 100 : null;
    return [
...
export async function buildAIGovernanceLifecycle(query, organizationId) {
...
export async function recordAICapabilityEvaluation(pool, input) {
    if (!pool)
        return null;
...
export async function buildPatient360Context(query, organizationId, patientId, task = 'clinical') {
    ...
    const context = { contextVersion: base.contextVersion, purpose: t, generatedAt: base.generatedAt };
...
export async function buildAISecurityIntelligence(query, organizationId, days = 30) {
...
export async function buildAIRiskIntelligence(query, organizationId) {
...
export async function buildClinicalVelocityIntelligence(query, organizationId, days = 30) {
    const d = Math.min(365, Math.max(1, Number(days) || 30));
...
export async function buildValueBasedCareIntelligence(query, organizationId, start, end) {
