...
export async function buildPatientIntelligenceLayer(query: Query, organizationId: string, patientId: string) {
  const [criticalLabs, urgentTasks, overdueFollowups, delayedReferrals, immunizationsDue, activeRisks, recentResults, admissions, carePlans, reconciliation,
    ...
    safe(() => query(`SELECT lr.id,lt.name AS "testName",lr.value_numeric AS "valueNumeric",lr.value_text AS "valueText",lr.unit,lr.status,ls.received_at AS "receivedAt" FROM lab_results lr JOIN lab_samples ls ON ls.id=lr.sample_id JOIN clinical_orders co ON co.id=ls.order_id JOIN lab_tests lt ON lt.id=lr.test_id JOIN patients p ON p.id=co.patient_id WHERE p.organization_id=$1 AND co.patient_id=$2 ORDER BY ls.received_at DESC LIMIT 12`, [organizationId, patientId]), []),
...
export async function buildClinicalVelocity(query: Query, organizationId: string, days = 30) {
  ...
  const metricRows = await safe(() => query(`
    SELECT
      ...
      (SELECT count(*)::int FROM clinical_orders co JOIN patients p ON p.id=co.patient_id WHERE p.organization_id=$1 AND co.created_at>=now()-($2::text||' days')::interval) AS "ordersCreated",
      (SELECT count(*)::int FROM lab_results lr JOIN lab_samples ls ON ls.id=lr.sample_id JOIN clinical_orders co ON co.id=ls.order_id JOIN patients p ON p.id=co.patient_id WHERE p.organization_id=$1 AND ls.received_at>=now()-($2::text||' days')::interval) AS "resultsAvailable",
  ...
  const [gaps, completedFollowups, completedReferrals, criticalResults] = await Promise.all([
    safe(() => query(`
      SELECT gap_type AS "gapType", severity, count(*)::int AS count FROM (
        SELECT 'critical-result-review' AS gap_type,'critical' AS severity FROM lab_results lr JOIN lab_samples ls ON ls.id=lr.sample_id JOIN clinical_orders co ON co.id=ls.order_id JOIN patients p ON p.id=co.patient_id WHERE p.organization_id=$1 AND lr.critical=true AND lr.status<>'released'
        UNION ALL
    ...
    safe(() => query(`SELECT count(*)::int AS count FROM lab_results lr JOIN lab_samples ls ON ls.id=lr.sample_id JOIN clinical_orders co ON co.id=ls.order_id JOIN patients p ON p.id=co.patient_id WHERE p.organization_id=$1 AND lr.critical=true AND lr.status='released' AND ls.received_at>=now()-($2::text||' days')::interval`, [organizationId, String(days)]), [{}]),
  ]);
  return {
...
export async function calculateClinicalMeasures(query: Query, organizationId: string, start: string, end: string) {
  const rows = await safe(() => query(`
    SELECT
      ...
      (SELECT count(*)::numeric FROM lab_results lr JOIN lab_samples ls ON ls.id=lr.sample_id JOIN clinical_orders co ON co.id=ls.order_id JOIN patients p ON p.id=co.patient_id WHERE p.organization_id=$1 AND lr.critical=true AND lr.status='released' AND ls.received_at::date BETWEEN $2::date AND $3::date) AS critical_release_num,
      (SELECT count(*)::numeric FROM lab_results lr JOIN lab_samples ls ON ls.id=lr.sample_id JOIN clinical_orders co ON co.id=ls.order_id JOIN patients p ON p.id=co.patient_id WHERE p.organization_id=$1 AND lr.critical=true AND ls.received_at::date BETWEEN $2::date AND $3::date AND lr.status IN ('verified','released')) AS critical_release_den
