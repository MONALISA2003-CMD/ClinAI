...
export type JourneyStep = { key:string; eventTypes:string[]; title:string; next?:string; maxMinutes:number };
export type JourneyDefinition = { key:string; title:string; steps:JourneyStep[] };
...
const journeyByEvent = new Map<string, {journey:JourneyDefinition; step:JourneyStep}>();
for (const journey of CLINICAL_JOURNEYS) for (const step of journey.steps) for (const event of step.eventTypes) journeyByEvent.set(event,{journey,step});
...
async function linkGraph(client:PoolClient,event:any,workflowId:string|null){
  const patientId=event.payload?.patientId; if(!patientId || !workflowId) return;
...
async function createGap(client:PoolClient, input:{organizationId:string;patientId:string;journey:JourneyDefinition;step:JourneyStep;sourceEventId:string;message:string;severity?:string;evidence?:any}){
...
async function closeGapIfProgressed(client:PoolClient, organizationId:string, patientId:string, journey:JourneyDefinition, step:JourneyStep){
...
async function directPharmacyAvailability(client:PoolClient,event:any){
  if(event.event_type!=='medication.ordered')return;
  const p=event.payload||{}; const medicationCode=String(p.medicationCode||'').trim(); const medicationName=String(p.medicationName||'').trim();
  if(!p.patientId || (!medicationCode&&!medicationName))return;
...
export async function processClinicalEvent(pool:Pool,event:any){
        ...
        if(next){
          ...
          if(prior.rowCount){
            ...
            if(age>next.maxMinutes*60000){
              ...
              if(!existsNext.rowCount) await createGap(client,{organizationId:event.organization_id,patientId:String(event.payload.patientId),journey,step:next,sourceEventId:event.id,severity:next.key==='safety-signal'?'high':'moderate',message:`${next.title} has not been recorded after ${step.title} within the configured workflow interval.`,evidence:{previousEvent:step.eventTypes,elapsedMinutes:Math.round(age/60000)}});
            }
          }
        }
      }
...
export async function scanUnfinishedJourneys(pool:Pool,organizationId:string,patientId?:string){
  ...
  for(const p of patients){
    for(const journey of CLINICAL_JOURNEYS){
      for(let i=0;i<journey.steps.length-1;i++){
        ...
        const age=Date.now()-new Date(prev[0].created_at).getTime(); if(age<next.maxMinutes*60000)continue;
