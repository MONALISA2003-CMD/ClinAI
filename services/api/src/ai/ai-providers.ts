...
export const AI_MODELS: AIModel[] = [
  ...
  { id: 'openai/gpt-oss-120b', provider: 'groq', label: 'GPT OSS 120B on Groq', tier: 'free', roles: ['general','reasoning','medical-reasoning','coding','fast'], context: 131072, multimodal: false, toolCalling: true, structuredOutput: true, publicEndpoint: false, noPersonalData: false, patientDataEligible: true, modalities: ['text'], enabled: true, priority: 5 },
...
export async function callOpenAICompatible(model: AIModel, prompt: string, system: string, schema: any, opts: { maxTokens?: number; reasoning?: boolean; tools?: any[]; messages?: any[]; attachments?: any[]; pool?: any; } = {}) {
  ...
  const timeoutMs = Math.max(3000, Number(process.env.CLINAI_PROVIDER_TIMEOUT_MS || 6000));
