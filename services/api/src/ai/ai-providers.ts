...
export const AI_MODELS: AIModel[] = [
  ...
  { id: 'openai/gpt-oss-120b', provider: 'groq', label: 'GPT OSS 120B on Groq', tier: 'paid', roles: ['reasoning','coding','fast'], context: 131072, multimodal: false, toolCalling: true, structuredOutput: true, publicEndpoint: false, enabled: true, priority: 30 },
...
export function selectModel(opts: { mode?: string; patientData?: boolean; preferredModel?: string; allowPublic?: boolean }) {
  ...
  const role = opts.mode === 'quick' ? 'fast' : opts.mode === 'research' ? 'research' : opts.mode === 'analysis' ? 'reasoning' : opts.mode === 'medical' ? 'medical-reasoning' : 'general';
...
export async function callOpenAICompatible(model: AIModel, prompt: string, system: string, schema: any, opts: { maxTokens?: number; reasoning?: boolean; tools?: any[]; messages?: any[] } = {}) {
  ...
  if (opts.reasoning) body.reasoning = { enabled: true };
  if (model.structuredOutput) body.response_format = { type: 'json_schema', json_schema: { name: 'clinai_answer', strict: true, schema } };
  const response = await fetch(url, { method: 'POST', headers: openAIHeaders(key, model.provider), body: JSON.stringify(body) });
