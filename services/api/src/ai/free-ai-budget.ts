...
export type FreeProvider = 'gemini' | 'openrouter' | 'groq';

type LimitConfig = { daily: number; perMinute: number; minIntervalMs: number };
type LocalState = { dayStartedAt: number; dailyUsed: number; minuteStartedAt: number; minuteUsed: number; lastRequestAt: number };

const LOCAL: Record<FreeProvider, LocalState> = {
...
const locks: Record<FreeProvider, Promise<void>> = { gemini: Promise.resolve(), openrouter: Promise.resolve(), groq: Promise.resolve() };
...
export function freeAIProviderLimits(provider: FreeProvider): LimitConfig {
...
function nowState(provider: FreeProvider) {
  const now = Date.now();
  const state = LOCAL[provider];
  if (now - state.dayStartedAt >= 24 * 60 * 60 * 1000) { state.dayStartedAt = now; state.dailyUsed = 0; }
  if (now - state.minuteStartedAt >= 60 * 1000) { state.minuteStartedAt = now; state.minuteUsed = 0; }
  return { now, state, limits: freeAIProviderLimits(provider) };
}

async function delay(ms: number) { if (ms > 0) await new Promise(resolve => setTimeout(resolve, ms)); }

async function reserveLocal(provider: FreeProvider) {
  const { now, state, limits } = nowState(provider);
  if (limits.daily <= 0 || limits.perMinute <= 0) throw Object.assign(new Error(`${provider} free-tier inference is disabled by ClinAI configuration.`), { code: 'AI_FREE_QUOTA_DISABLED', statusCode: 503, provider });
  if (state.dailyUsed >= limits.daily) throw Object.assign(new Error(`${provider} free-tier safety budget is exhausted.`), { code: 'AI_FREE_QUOTA_EXHAUSTED', statusCode: 429, provider });
  if (state.minuteUsed >= limits.perMinute) throw Object.assign(new Error(`${provider} free-tier per-minute safety budget is exhausted.`), { code: 'AI_FREE_RATE_LIMITED', statusCode: 429, provider });
...
async function reserveDatabase(pool: Pool, provider: FreeProvider) {
  ...
  try {
    ...
    if (now - dayStart >= 24 * 60 * 60 * 1000) { dailyUsed = 0; effectiveDayStart = new Date().toISOString(); }
    if (now - minuteStart >= 60 * 1000) { minuteUsed = 0; effectiveMinuteStart = new Date().toISOString(); }
    if (limits.daily <= 0 || limits.perMinute <= 0) throw Object.assign(new Error(`${provider} free-tier inference is disabled by ClinAI configuration.`), { code: 'AI_FREE_QUOTA_DISABLED', statusCode: 503, provider });
    if (dailyUsed >= limits.daily) throw Object.assign(new Error(`${provider} free-tier safety budget is exhausted.`), { code: 'AI_FREE_QUOTA_EXHAUSTED', statusCode: 429, provider });
    if (minuteUsed >= limits.perMinute) throw Object.assign(new Error(`${provider} free-tier per-minute safety budget is exhausted.`), { code: 'AI_FREE_RATE_LIMITED', statusCode: 429, provider });
...
export async function reserveFreeAIRequest(pool: Pool | null, provider: FreeProvider) {
  let release!: () => void;
  const prior = locks[provider];
  locks[provider] = new Promise<void>(resolve => { release = resolve; });
  await prior.catch(() => undefined);
  try {
    // The database is the authoritative cross-instance limiter when available.
    if (pool) await reserveDatabase(pool, provider);
    else await reserveLocal(provider);
  } finally {
    release();
  }
}

export async function freeAIQuotaStatus(pool: Pool | null) {
  const providers: Record<string, any> = {};
  for (const provider of ['gemini','openrouter','groq'] as FreeProvider[]) {
    const limits = freeAIProviderLimits(provider);
