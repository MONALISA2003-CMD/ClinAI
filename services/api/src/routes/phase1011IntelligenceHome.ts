...
export function registerPhase1011Routes(app:any,pool:Pool|null,deps:Phase1011Deps){
  ...
  app.get('/api/phase10/intelligence/:workspace', async (req:any, reply:any)=>{
    if(!pool)return reply.code(503).send({error:'Intelligence requires PostgreSQL.'});
    ...
    try{
      ...
      if(workspace==='clinical-velocity'){
        const [series,bottlenecks]=await Promise.all([
          pool.query(`WITH d AS (SELECT generate_series(current_date-($2::int-1),current_date,interval '1 day')::date day) SELECT d.day,COALESCE((SELECT count(*) FROM clinical_workflow_events e WHERE e.organization_id=$1 AND e.created_at::date=d.day),0)::int AS signals,COALESCE((SELECT count(*) FROM clinical_workflow_events e WHERE e.organization_id=$1 AND e.event_type IN ('result.verified','result.released','task.completed','referral.completed','followup.completed') AND e.created_at::date=d.day),0)::int AS actions,COALESCE((SELECT count(*) FROM encounters e WHERE e.organization_id=$1 AND e.started_at::date=d.day),0)::int AS encounters FROM d ORDER BY d.day`,[organizationId,days]),
  ...
  app.post('/api/phase10/intelligence/:workspace/action', async(req:any,reply:any)=>{
    if(!pool)return reply.code(503).send({error:'Intelligence actions require PostgreSQL.'});
  ...
  app.get('/api/phase11/home',async(req:any,reply:any)=>{
    if(!pool)return reply.code(503).send({error:'Command Center requires PostgreSQL.'});
    const organizationId=org(req); if(!organizationId)return reply.code(400).send({error:'Organization context is required.'});
    try{
