    ...
    try{
      ...
      if(workspace==='patient-360'){
        ...
        const [events,diagnoses,notes,orders,labs,imaging,meds,recon,referrals,admissions,beds,nursing,surgery,maternity,pediatrics,chronic,insurance,invoices,payments,gaps,signals]=await Promise.all([
          ...
          pool.query(`SELECT d.id,d.code,d.display,d.diagnosis_type AS "diagnosisType",d.status,d.onset_date AS "onsetDate",d.resolution_date AS "resolutionDate" FROM diagnoses d JOIN patients p ON p.id=d.patient_id WHERE p.organization_id=$1 AND d.patient_id=$2 ORDER BY d.onset_date DESC NULLS LAST,d.id DESC LIMIT 40`,[organizationId,patientId]),
          pool.query(`SELECT cn.id,cn.note_type AS "noteType",cn.assessment,cn.plan,cn.created_at AS "createdAt" FROM clinical_notes cn JOIN encounters e ON e.id=cn.encounter_id WHERE e.organization_id=$1 AND e.patient_id=$2 ORDER BY cn.created_at DESC LIMIT 30`,[organizationId,patientId]),
          ...
          pool.query(`SELECT id,procedure_name AS "procedureName",scheduled_at AS "scheduledAt",status,consent_confirmed AS "consentConfirmed",updated_at AS "updatedAt" FROM surgery_cases WHERE organization_id=$1 AND patient_id=$2 ORDER BY COALESCE(scheduled_at,created_at) DESC LIMIT 20`,[organizationId,patientId]),
          ...
          pool.query(`SELECT ip.id,ip.policy_number AS "policyNumber",ip.status,ip.provider_id AS "providerId",ip.effective_from AS "effectiveFrom",ip.effective_to AS "effectiveTo" FROM insurance_policies ip WHERE ip.organization_id=$1 AND ip.patient_id=$2 ORDER BY ip.effective_from DESC NULLS LAST,ip.id DESC LIMIT 10`,[organizationId,patientId]),
      ...
      if(workspace==='ai-governance'){
        const [caps,evals,feedback,usage,incidents,lifecycle]=await Promise.all([
          ...
          pool.query(`SELECT purpose AS "eventType",count(*)::int AS count FROM ai_usage_events WHERE organization_id=$1 GROUP BY purpose ORDER BY count DESC LIMIT 20`,[organizationId]),
