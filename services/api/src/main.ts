...
app.get('/api/public/test-patients/:patientNumber/360',async(req:any,reply:any)=>{
  if(!pool)return reply.code(503).send({error:'Patient 360 is temporarily unavailable.'});
...
app.get('/api/public/test-modules/:module',async(req:any,reply:any)=>{
  if(!pool)return reply.code(503).send({error:'Clinical data is temporarily unavailable.'});
  if(!PUBLIC_TEST_DATA_ENABLED)return reply.code(404).send({error:'Public test data is disabled.'});
...
app.get('/api/public/test-intelligence/:workspace',async(req:any,reply:any)=>{
  if(!pool)return reply.code(503).send({error:'Clinical intelligence is temporarily unavailable.'});
  if(!PUBLIC_TEST_DATA_ENABLED)return reply.code(404).send({error:'Public test data is disabled.'});
...
app.post('/api/public/test-session',async(req:any,reply:any)=>{
  if(!pool)return reply.code(503).send({error:'The public clinical workspace is temporarily unavailable.'});
  if(!PUBLIC_TEST_DATA_ENABLED)return reply.code(403).send({error:'Public test data is currently disabled.'});
  try{
...
app.post('/api/patients/:id/identifiers',async(req:any,reply)=>{
  if(!pool) return reply.code(501).send({error:'Patient identifiers are temporarily unavailable.'});
...
app.get('/api/fhir/Encounter/:id',async(req:any,reply)=>{
  if(!pool)return reply.code(501).send({error:'FHIR encounter data is temporarily unavailable.'});
...
app.post('/api/check-in/walk-in',async(req:any,reply)=>{
  if(!pool)return reply.code(501).send({error:'Clinical data service is unavailable.'}); const b=z.object({patientId:z.string().uuid(),priority:z.enum(['routine','urgent','emergency','stat']).default('routine'),service:z.string().default('general')}).parse(req.body||{});
...
app.post('/api/cds-services/:serviceId',async(req:any,reply:any)=>{
  ...
  const organizationId=dbOrganizationId(req); if(!organizationId)return reply.code(400).send({error:'Organization context is required'});
  if(!pool)return reply.code(501).send({error:'CDS service is temporarily unavailable'});
...
registerPhase1011Routes(app,pool,{dbOrganizationId,dbUserId,requireAuthorizedWrite,dbAudit});
