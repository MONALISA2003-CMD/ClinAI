...
// --- Patient 360 and Uganda-aware patient identity ---
app.get('/api/patients/:id/360',async(req:any,reply)=>{
  if(!pool) return reply.code(501).send({error:'Patient 360 requires PostgreSQL'});
  const oid=dbOrganizationId(req), pid=req.params.id;
  const patient=await pool.query(`SELECT p.id,p.organization_id AS "organizationId",p.facility_id AS "facilityId",p.patient_number AS "patientNumber",p.first_name AS "firstName",p.middle_name AS "middleName",p.last_name AS "lastName",p.date_of_birth AS "dateOfBirth",p.sex,p.phone,p.email,p.address,p.national_identifier AS "nationalId",p.preferred_language AS "preferredLanguage",p.status,p.created_at AS "createdAt",p.updated_at AS "updatedAt" FROM patients p WHERE p.id=$1 AND p.organization_id=$2`,[pid,oid]);
  if(!patient.rowCount) return reply.code(404).send({error:'Patient not found'});
  const [identifiers,contacts,emergencyContacts,allergies,appointments,encounters,observations,diagnoses,procedures,orders,medications,dispensations,administrations,carePlans,referrals,followUp,alerts,clinicalNotes,notifications,admissions,nursing,surgery,maternity,pediatrics,immunizations,chronic,telemedicine,monitoring]=await Promise.all([
    pool.query(`SELECT id,system,value,identifier_type AS "identifierType",use,period_start AS "periodStart",period_end AS "periodEnd" FROM patient_identifiers WHERE patient_id=$1 AND organization_id=$2 ORDER BY created_at DESC`,[pid,oid]),
    pool.query(`SELECT id,type,value,is_primary AS "isPrimary" FROM patient_contacts WHERE patient_id=$1 ORDER BY is_primary DESC`,[pid]),
    pool.query(`SELECT id,name,relationship,phone,address FROM emergency_contacts WHERE patient_id=$1 ORDER BY id`,[pid]),
    pool.query(`SELECT id,substance,reaction,severity,status FROM allergies WHERE patient_id=$1 ORDER BY id DESC`,[pid]),
    pool.query(`SELECT id,start_at AS "startAt",end_at AS "endAt",type,status,reason,provider_user_id AS "providerUserId",facility_id AS "facilityId" FROM appointments WHERE patient_id=$1 AND organization_id=$2 ORDER BY start_at DESC LIMIT 100`,[pid,oid]),
    pool.query(`SELECT id,facility_id AS "facilityId",provider_user_id AS "providerUserId",appointment_id AS "appointmentId",type,status,started_at AS "startedAt",ended_at AS "endedAt" FROM encounters WHERE patient_id=$1 AND organization_id=$2 ORDER BY started_at DESC LIMIT 100`,[pid,oid]),
    pool.query(`SELECT id,encounter_id AS "encounterId",code_system AS "codeSystem",code,display,value_numeric AS "valueNumeric",value_text AS "valueText",unit,observed_at AS "observedAt" FROM observations WHERE patient_id=$1 ORDER BY observed_at DESC LIMIT 200`,[pid]),
    pool.query(`SELECT id,encounter_id AS "encounterId",code_system AS "codeSystem",code,display,diagnosis_type AS "diagnosisType",status FROM diagnoses WHERE patient_id=$1 ORDER BY id DESC LIMIT 100`,[pid]),
    pool.query(`SELECT id,encounter_id AS "encounterId",note_type AS "noteType",subjective,objective,assessment,plan,signed_by AS "signedBy",signed_at AS "signedAt",version FROM clinical_notes WHERE encounter_id IN (SELECT id FROM encounters WHERE patient_id=$1) ORDER BY id DESC LIMIT 100`,[pid]),
    pool.query(`SELECT id,encounter_id AS "encounterId",code_system AS "codeSystem",code,display,performed_at AS "performedAt",performer_user_id AS "performerUserId" FROM procedures WHERE patient_id=$1 ORDER BY performed_at DESC NULLS LAST LIMIT 100`,[pid]),
    pool.query(`SELECT id,encounter_id AS "encounterId",order_type AS "orderType",priority,status,details,created_at AS "createdAt" FROM clinical_orders WHERE patient_id=$1 ORDER BY created_at DESC LIMIT 200`,[pid]),
    pool.query(`SELECT mo.id,mo.encounter_id AS "encounterId",mo.medication_id AS "medicationId",m.code,m.name,m.strength,m.form,mo.dose,mo.frequency,mo.route,mo.duration,mo.quantity,mo.status,mo.prescribed_by AS "prescribedBy" FROM medication_orders mo JOIN medications m ON m.id=mo.medication_id WHERE mo.patient_id=$1 ORDER BY mo.id DESC LIMIT 100`,[pid]),
    pool.query(`SELECT d.id,d.medication_order_id AS "medicationOrderId",d.quantity,d.batch,d.expiry_date AS "expiryDate",d.dispensed_by AS "dispensedBy",d.dispensed_at AS "dispensedAt" FROM dispensations d JOIN medication_orders mo ON mo.id=d.medication_order_id WHERE mo.patient_id=$1 ORDER BY d.dispensed_at DESC LIMIT 100`,[pid]),
    pool.query(`SELECT ma.id,ma.medication_order_id AS "medicationOrderId",ma.dose,ma.route,ma.scheduled_at AS "scheduledAt",ma.administered_at AS "administeredAt",ma.status,ma.administered_by AS "administeredBy",ma.reason FROM medication_administrations ma JOIN medication_orders mo ON mo.id=ma.medication_order_id WHERE mo.patient_id=$1 ORDER BY COALESCE(ma.administered_at,ma.scheduled_at) DESC LIMIT 100`,[pid]),
    pool.query(`SELECT id,encounter_id AS "encounterId",title,status,goals FROM care_plans WHERE patient_id=$1 ORDER BY id DESC`,[pid]),
    pool.query(`SELECT id,encounter_id AS "encounterId",destination,reason,status,created_at AS "createdAt" FROM referrals WHERE patient_id=$1 ORDER BY created_at DESC LIMIT 100`,[pid]),
    pool.query(`SELECT id,channel,template,status,payload,scheduled_at AS "scheduledAt",sent_at AS "sentAt",created_at AS "createdAt" FROM notifications WHERE patient_id=$1 ORDER BY created_at DESC LIMIT 100`,[pid]),
    pool.query(`SELECT id,status,payload,created_at AS "createdAt" FROM module_records WHERE organization_id=$1 AND module='follow-up' AND payload->>'patientId'=$2 ORDER BY created_at DESC LIMIT 100`,[oid,pid]),
    pool.query(`SELECT id,status,payload,created_at AS "createdAt" FROM module_records WHERE organization_id=$1 AND module='clinical-alerts' AND payload->>'patientId'=$2 ORDER BY created_at DESC LIMIT 100`,[oid,pid]),
    pool.query(`SELECT id,encounter_id AS "encounterId",ward,bed,status,admitted_at AS "admittedAt",discharged_at AS "dischargedAt",discharge_summary AS "dischargeSummary" FROM admissions WHERE patient_id=$1 AND organization_id=$2 ORDER BY admitted_at DESC`,[pid,oid]),
    pool.query(`SELECT id,encounter_id AS "encounterId",nurse_user_id AS "nurseUserId",assessment_type AS "assessmentType",findings,pain_score AS "painScore",risk_flags AS "riskFlags",status,created_at AS "createdAt" FROM nursing_assessments WHERE patient_id=$1 AND organization_id=$2 ORDER BY created_at DESC LIMIT 100`,[pid,oid]),
    pool.query(`SELECT id,encounter_id AS "encounterId",procedure_name AS "procedureName",scheduled_at AS "scheduledAt",status,consent_confirmed AS "consentConfirmed",checklist,surgeon_user_id AS "surgeonUserId",anaesthesia_plan AS "anaesthesiaPlan" FROM surgery_cases WHERE patient_id=$1 AND organization_id=$2 ORDER BY scheduled_at DESC NULLS LAST LIMIT 100`,[pid,oid]),
    pool.query(`SELECT id,encounter_id AS "encounterId",pregnancy_id AS "pregnancyId",event_type AS "eventType",gestational_age_weeks AS "gestationalAgeWeeks",gravida,para,maternal_observations AS "maternalObservations",fetal_observations AS "fetalObservations",status,event_at AS "eventAt",notes FROM maternity_records WHERE patient_id=$1 AND organization_id=$2 ORDER BY event_at DESC LIMIT 100`,[pid,oid]),
    pool.query(`SELECT id,encounter_id AS "encounterId",age_months AS "ageMonths",weight_kg AS "weightKg",height_cm AS "heightCm",muac_mm AS "muacMm",temperature,respiratory_rate AS "respiratoryRate",spo2,assessment,imci_classification AS "imciClassification",nutrition_status AS "nutritionStatus",referral_required AS "referralRequired",created_at AS "createdAt" FROM pediatric_assessments WHERE patient_id=$1 AND organization_id=$2 ORDER BY created_at DESC LIMIT 100`,[pid,oid]),
    pool.query(`SELECT id,vaccine_code AS "vaccineCode",vaccine_name AS "vaccineName",dose_number AS "doseNumber",administered_at AS "administeredAt",lot_number AS "lotNumber",expiry_date AS "expiryDate",route,site,vaccinator_user_id AS "vaccinatorUserId",status,next_due_at AS "nextDueAt",notes FROM immunizations WHERE patient_id=$1 AND organization_id=$2 ORDER BY administered_at DESC`,[pid,oid]),
    pool.query(`SELECT id,encounter_id AS "encounterId",condition_code AS "conditionCode",condition_name AS "conditionName",status,goals,measures,medications,next_review_at AS "nextReviewAt",risk_level AS "riskLevel",care_plan AS "carePlan" FROM chronic_care_records WHERE patient_id=$1 AND organization_id=$2 ORDER BY updated_at DESC`,[pid,oid]),
    pool.query(`SELECT id,appointment_id AS "appointmentId",encounter_id AS "encounterId",scheduled_at AS "scheduledAt",status,meeting_url AS "meetingUrl",identity_verified AS "identityVerified",consent_confirmed AS "consentConfirmed",started_at AS "startedAt",ended_at AS "endedAt",provider_user_id AS "providerUserId",notes FROM telemedicine_sessions WHERE patient_id=$1 AND organization_id=$2 ORDER BY scheduled_at DESC LIMIT 100`,[pid,oid]),
    pool.query(`SELECT id,device_id AS "deviceId",metric,value_numeric AS "valueNumeric",unit,measured_at AS "measuredAt",source,validation_status AS "validationStatus",alert_status AS "alertStatus",metadata FROM remote_monitoring_readings WHERE patient_id=$1 AND organization_id=$2 ORDER BY measured_at DESC LIMIT 200`,[pid,oid])
  ]);
  return {patient:patient.rows[0],identifiers:identifiers.rows,contacts:contacts.rows,emergencyContacts:emergencyContacts.rows,allergies:allergies.rows,appointments:appointments.rows,encounters:encounters.rows,observations:observations.rows,diagnoses:diagnoses.rows,procedures:procedures.rows,orders:orders.rows,medications:medications.rows,dispensations:dispensations.rows,medicationAdministrations:administrations.rows,carePlans:carePlans.rows,referrals:referrals.rows,followUp:followUp.rows,clinicalAlerts:alerts.rows,clinicalNotes:clinicalNotes.rows,notifications:notifications.rows,admissions:admissions.rows,nursing:nursing.rows,surgery:surgery.rows,maternity:maternity.rows,pediatrics:pediatrics.rows,immunizations:immunizations.rows,chronicCare:chronic.rows,telemedicine:telemedicine.rows,remoteMonitoring:monitoring.rows,meta:{country:'UG',currency:'UGX',sourceOfTruth:'PostgreSQL',generatedAt:now()}};
});
