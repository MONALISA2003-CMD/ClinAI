...
app.get('/api/patients/:id/360',async(req:any,reply)=>{
  ...
  const [contacts,emergency,allergies,appointments,encounters,orders,diagnoses,observations,notes,medications,notifications,admissions,immunizations,chronicCare,telemedicine,remoteMonitoring,carePlans,referrals,followUp,careTasks,clinicalAlerts]=await Promise.all([
    ...
    pool.query(`SELECT id,COALESCE(payload->>'kind',module) AS kind,COALESCE(payload->>'severity','') AS severity,status,payload,created_at AS "createdAt" FROM module_records WHERE patient_id=$1 AND module IN ('clinical-alerts','care-gaps') AND status NOT IN ('resolved','closed','completed') ORDER BY created_at DESC LIMIT 50`,[id])
  ]);
