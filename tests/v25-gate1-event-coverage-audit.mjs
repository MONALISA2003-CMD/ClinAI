...
const required = [
  ...
  ['medication order event', 'CLINICAL_EVENT_TYPES.MEDICATION_ORDERED'],
...
for (const [name, token] of required) {
  if (!main.includes(token) && !events.includes(token)) throw new Error(`Missing Gate 1 contract: ${name}`);
}
