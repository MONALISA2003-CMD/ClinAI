...
assert.match(src, /const \[summary,recent,recon\]=await Promise\.all\(\[/, 'Finance overview must destructure exactly its three Promise.all results.');
assert.doesNotMatch(src, /const \[summary,recent,recon,payments\]=await Promise\.all\(\[/, 'Regression: finance overview must not destructure a fourth Promise.all result.');
