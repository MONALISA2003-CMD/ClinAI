...
const tsc = spawnSync('tsc',['-p','/tmp/clinai-ts-harness/tsconfig.json'],{encoding:'utf8'});
ok('TSX semantic harness', tsc.status===0, (tsc.stdout||'')+(tsc.stderr||''));
const routeTsc = spawnSync('tsc',['-p','/tmp/clinai-ts-harness/route-tsconfig.json'],{encoding:'utf8'});
