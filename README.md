...
ClinAI includes a provider-independent AI router with Gemini plus multiple OpenRouter free models, Groq and Cerebras adapters. It supports model selection, fallback, provider audit logging, deterministic Python calculations, and a shared ClinAI tool gateway. OpenRouter public/free AI endpoints are blocked from patient clinical context by default. The router also enforces hard capability matching so a text-only model cannot be selected for an explicitly multimodal request. `CLINAI_ALLOW_PUBLIC_AI_WITH_PATIENT_DATA` remains false by default and does not expose record-level patient context to public models.
