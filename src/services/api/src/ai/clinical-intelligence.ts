...
const daysOpen = (v: any) => {
  const d = asDate(v);
  return d ? Math.max(0, (Date.now() - d.getTime()) / 86400000) : 0;
};

function compact(value: any, max = 220) {
  const s = typeof value === 'string' ? value : JSON.stringify(value ?? '');
  return s.length > max ? `${s.slice(0, max)}…` : s;
}

function medicationAllergyConflicts(context: Row) {
  const conflicts: Row[] = [];
...
function duplicateMedications(context: Row[]) {
...
function imagingGaps(context: Row) {
...
function appointmentEncounterGaps(context: Row) {
...
function referralGaps(context: Row) {
...
function followupGaps(context: Row) {
...
function observationTrends(context: Row) {
  const groups = new Map<string, Row[]>();
  for (const o of arr(context.observations)) {
    const key = text(o.code || o.display);
    if (!key || o.valueNumeric === null || o.valueNumeric === undefined) continue;
    const list = groups.get(key) || [];
    list.push(o);
    groups.set(key, list);
  }
  ...
  for (const [key, list] of groups) {
    ...
    if (!Number.isFinite(first) || !Number.isFinite(latest) || first === latest) continue;
...
export function buildPatientIntelligence(context: Row) {
  const consistency = [
    ...labGaps(context),
    ...imagingGaps(context),
...
export function buildEvidenceIndex(context: Row) {
  const evidence: Row[] = [];
  const add = (module: string, rows: any[], idKey = 'id', dateKey?: string, label?: (x: Row) => string) => {
...
export function buildQuestionIntent(question: string) {
...
export function compactIntelligenceForPrompt(intelligence: Row) {
  return {
